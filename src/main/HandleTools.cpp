...
#include "interface/dimreduct/DimReductCtrlDlg.h"
#include "interface/dimreduct/partition/RddtClustEvlDlg.h"
#include "interface/dimreduct/heatmap/ClustHeatMapViewDlg.h"
...
HandleTools::HandleTools(XmdvToolMainWnd* mainWnd){
    ...
    chvDlg = NULL; evlDlg = NULL; sbbDlg=NULL;
    ...
    connect ( this, SIGNAL(dimReductModeEnabled()),
              this, SLOT(openDimReductDlg()) );
}
...
HandleTools::~HandleTools() {
    ...
    SAFE_DELETE(evlDlg);
    SAFE_DELETE(sbbDlg);
    //SAFE_DELETE(scatterplotManager);
}
...
void HandleTools::openDimReductDlg() {
	...
	//TODO: change this to DimreductControl;
    sbbDlg = new DimReductCtrlDlg(m_mainWnd,
			m_mainWnd->getPipelineManager(), win->getPipelineID());
	sbbDlg->show();
	//InterringControl *sbbDlg = new InterringControl(m_mainWnd,
	//		m_mainWnd->getPipelineManager(), win->getPipelineID());
	//sbbDlg->show();

    evlDlg = new RddtClustEvlDlg(m_mainWnd,
            m_mainWnd->getPipelineManager(), win->getPipelineID());
    evlDlg->show();

    chvDlg = new ClustHeatMapViewDlg(m_mainWnd,
            m_mainWnd->getPipelineManager(), win->getPipelineID());
    chvDlg->show();

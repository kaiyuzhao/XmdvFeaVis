...
char* OkcData::fgetcsvline(vector<string> &csv_databuf, fstream* fhead) {
    char  *ret_stat;
    //char  data_buf[1024];
    ...
    string buf;
    if(!(*fhead).eof()){
        std::getline((*fhead),buf);
        ret_stat = (char*)buf.c_str();
    }else{
        ret_stat = NULL;
        return NULL;
    }

    char* data_buf = (char*)buf.c_str();

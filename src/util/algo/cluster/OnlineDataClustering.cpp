...
/*
void Clustering(int n, list<ClusterNode*> & pointset, list<ClusterNode*> & clusterset, double min_distance)
{
    ...
    for ( node1 = pointset.begin(); node1 != pointset.end(); ++node1 )
    {
		...
		// PAUL: cluster_org
		bool skip_old = (*node1)->min_distance > min_distance;
		double new_min_distance = (*node1)->min_distance;
        ...
        for (; node2 != pointset.end(); ++node2 )
        {
			...
			double diff = (data2[InitialClusterSortDim] - data1[InitialClusterSortDim]);
			if ( diff > min_distance )
			{
				new_min_distance = MIN(new_min_distance, diff);
				break;	// No use searching after this point
			}

            flag = true;
			// PAUL: cluster_org
			// Start at i = 1 because we already checked i = 0
            for ( i = 0; i < n; i++ )
            {
				if (i == InitialClusterSortDim)
					continue;	// This dimension was already checked

				diff = (data1[i] > data2[i] ? data1[i] - data2[i] : data2[i] - data1[i]);
				if (diff > min_distance)
                {
                    new_min_distance = MIN(new_min_distance, diff);
                    flag = false;
		...
		(*node1)->min_distance = new_min_distance;
		(*node1)->firsttime = false;
    }

	// PAUL: test
	printf("****** Done with Pairings (%d) *****\n", data_pairs.size());

    while ( (center = WhoIsMaximum(pointset)) != NULL && (*center)->count > 0 )
    {
        ...
        new_clusternode->parent = NULL;
		new_clusternode->firsttime = true;
